...
namespace PhysicsEngine
{
	...
	public:
		...
		ConvexMesh(const PxTransform& pose = PxTransform(PxIdentity), PxReal density = 1.f)
			: T(pose)
		{
			_density = density;
		}

		void AddVerts(const std::vector<PxVec3>& verts)
		{
			...
			CreateShape(PxConvexMeshGeometry(CookMesh(mesh_desc)), _density);
		}
